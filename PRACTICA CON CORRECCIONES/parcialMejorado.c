...
int busquedaRepetidos(int tamano, int matriz[][tamano], int numero){
...
int main()
{
    ...
    for (i=0;i<tamano;i++){
        for (j=0;j<tamano;j++){
            do {
                numero = rand () %99 + 0;
            } while (busquedaRepetidos(tamano,carton,numero)==1);

            if (busquedaRepetidos(tamano,carton,numero)==0){
                carton[i][j]=numero;
            }
        }
    }
