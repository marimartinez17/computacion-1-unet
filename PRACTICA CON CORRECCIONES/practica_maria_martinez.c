...
int main (){
    ...
    for(i=0;i<cantidadLibros;i++){
        if (ganancia[i] >= 0) {
            ...
            printf("\n|%-16s|%-16s|%-16s|%-16s\%-16.2f|%-16d|\033[0;32m%-16.2f\033[0m|", titulo[i],autor[i],genero[i], isbn[i],precio[i],stock[i],ganancia[i]);
        }
        if (ganancia[i] < 0){

            //SALIDA SI EL VALOR DE LA GANANCIA ES NEGATIVO (COLOR ROJO)

            printf("\n|%-16s|%-16s|%-16s|%-16s|%-16.2f|%-16d|\033[0;31m%-16.2f\033[0m|", titulo[i],autor[i],genero[i], isbn[i],precio[i],stock[i],ganancia[i]);
        }
    }
    return 0;
}
